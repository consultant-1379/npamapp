 ...
 */
class NeAccountTable implements PageFragment {
...
//    @FindBy(css = ".elTablelib-Table-wrapper .elTablelib-Table-body > tr:nth-child(1) > td:nth-child(1)")
    @FindBy(css = ".elTablelib-Table-pretable tbody> tr:nth-child(1) > td:nth-child(1)")
    WebElement firstCheckBoxInNeAccoutTable

    // NOT OK as selectors
  /*  @FindBy(className = "eaCellManagement-wTableHeader-totalRows")
    WebElement totalRowsSpan

    @FindBy(className = "eaCellManagement-wTableHeader-selectedRows-count")
    WebElement selectedRowsSpan
*/

//    @FindBy(css = ".elNetworkObjectLib-rTopologyHeader-clearSelection-Link")
//    WebElement clearLink

    @FindBy(css = ".eaNpamapp-NeAccountTable-SectionHeading-selected-clear-link")
    WebElement clearLink

    // NOT FOUND - provisionally commented out
   // @FindBy(css = ".elTablelib-VirtualScrolling-loader")
  //  WebElement virtualScrollBar
...
//            logger.error("Sort icon not visible {}", e);
    ...
    boolean noErrorIcon(final int rowIndex, final String columnName) {
        List<WebElement> tableRows = tableContent.get(0).findElements(By.xpath("tr"));
        List<WebElement> tableCells = tableRows.get(rowIndex).findElements(By.xpath("td"));
        int colIndex = getColumnIndex(columnName);
        return tableCells.get(colIndex).findElement(By.id("resultCellError")).getCssValue("display") == "none"
    }

    boolean noWarningIcon(final int rowIndex, final String columnName) {
        List<WebElement> tableRows = tableContent.get(0).findElements(By.xpath("tr"));
        List<WebElement> tableCells = tableRows.get(rowIndex).findElements(By.xpath("td"));
        int colIndex = getColumnIndex(columnName);
        return tableCells.get(colIndex).findElement(By.id("resultCellWarning")).getCssValue("display") == "none"
    }
